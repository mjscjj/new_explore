// 豆包 前端 client（独立模块）。只负责连到本项目后端的 /ws/doubao，
// 遵守 CONTRACT.md 的统一线路协议。不含任何豆包上游/协议细节（那在后端）。
export class DoubaoClient {
  ...
  get name() { return "doubao"; }

  connect(config = {}) {
    const proto = location.protocol === "https:" ? "wss" : "ws";
    this.ws = new WebSocket(`${proto}://${location.host}/ws/doubao`);
