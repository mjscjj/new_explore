// QwebO（Qwen-Omni 全模态实时）前端 client（独立模块）。只负责连到本项目后端的 /ws/qwebo，
// 遵守 CONTRACT.md 的统一线路协议。不含任何上游细节（那在后端）。
export class QweboClient {
  constructor(handlers) {
    this.h = handlers || {};
    this.ws = null;
  }

  get name() { return "qwebo"; }

  connect(config = {}) {
    const proto = location.protocol === "https:" ? "wss" : "ws";
    this.ws = new WebSocket(`${proto}://${location.host}/ws/qwebo`);
